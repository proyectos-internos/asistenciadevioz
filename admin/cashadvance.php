          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    $sql = "SELECT *, a.id AS caid, e.empleado_id AS empid FROM adelanto_efecivo as a LEFT JOIN empleado as e ON e.id=a.empleado_id ORDER BY fecha_advance DESC";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                          <td class='hidden'></td>
                          <td>".date('M d, Y', strtotime($row['fecha_advance']))."</td>
                          <td>".$row['empid']."</td>
